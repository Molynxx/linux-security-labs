...
- plik `/etc/shadow` to plik przechowujące hashe haseł i wszelkie informacje dotyczące ważności haseł, rodzaju użytego algorytmu hashującego, blokady haseł, itp. Dlatego ważne jest, żeby dostęp do tego pliku był ograniczony. Wyłącznie root powinien móc odczytać i zapisać hasła. 
...
- aby ustawić poprane ustawienia w przypadku gdy nie są poprawne należy użyć polecenia: `chmod 600 /etc/shadow`. 

## Case study 3
`SUID` na vim - ktoś ustawił `chmod u+s /usr/bin/vim`. Czy to bezpieczne?  
...
- by wyszukać i przejrzeć pliki, które mają ustawiony `SUID` należy użyć polecenia: ` find / -perm -4000 2>/dev/null`. 
...
- ustawienie `SGID` bywa użyteczne, gdy istnieje potrzeba by zwykły użytkownik wykonał polecenie z podwyższonymi uprawnieniami np. może to być skrypt zawierający polecenie `sudo apt upgrade`. Należy jednak być ostrożnym w nadawaniu katalogom `SGID`, by nie spowodować eskalacji uprawnień w kluczowych dla bezpieczeństwa systemu plikach. 
