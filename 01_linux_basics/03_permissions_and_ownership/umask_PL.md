...
- dla plików domyślnie ustawione są uprawnienia 666 (rw-rw-rw), lecz umask odejmuje od tego określone bity. Przykład `umask 022` spowoduje ustawienie uprawnień dla nowo tworzonych plików na 644 (rw-r--r--).
...
- `/etc/profile`, `~/.bashrc`, `~/.profile` - ustawia umask dla całej sesji. UWAGA: nadpisuje PAM,
...
- należy ustawiać `umask` zgodnie do dobrą praktyką, tj.:
