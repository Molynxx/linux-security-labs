...
## Cel laboratorium 
Celem tego laboratorium było zapoznanie się z tym, jak działają zapisy w pliku `/etc/sudoers` oraz jaki mają wpływ na uprawnienia użytkowników.
...
## Obserwacje 
- UID 0 (root) ma pełne uprawnienia bez użycia sudo, natomiast użytkownicy z grupy sudo uzyskują pełne uprawnienia za pomocą polecenia `sudo`, zgodnie z konfiguracją pliku `/etc/sudoers`,
