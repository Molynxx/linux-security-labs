...
## Etapy bootowania
- `BIOS/UEFI` - na tym etapie sprawdzane jest poprawne działanie sprzętu (POST - Power-_On Self Test), jeśli wszystko jest w porządku, płyta główna nie wydaje ostrzeżenia o błędzie -> szuka bootloadera na dysku,
- `Bootloader (GRUB)` - ładuje jądro i initramfs, pokazuje menu do wyboru systemu,
- `Kernel` - Jądro przemuje kontrolę, inicjalizuje sprzęt oraz montuje initramfs,
...
- `initramfs`:
	- zagrożenie: jest to tymczasowy system plików, który jest ładowany do pamięci RAM razem z jądrem. Zawiera skrypty oraz sterowniki niezbędne do zamontowania właściwego systemu plików z dysku. `initramfs` nie jest plikiem, to jest archiwum, zawierające cały katalog ze skryptami i programami. Jego główny skrypt startowy nosi nazwę `init` i wykonuje się jako pierwszy. Atakujący (mający dostęp do root) może:  
		...
		- czytać rozpakowaną zawartość pliku (na niektóych systemach jest `initramfs.img`  anie `initrd.img`),
		- dodać swój skrypt, 
...
- `kernel`:
	- zagrożenie: podmiana jądra systemu (rootkit jądra). Atakujący jeśli ma dostęp do roota"
		...
		Rootkit na poziomie jądra to program który:
			...
			Należy pamiętać ze programy takie jak `ps`, `ls`, `netstat` działają w przestrzeni użytkownika. Żeby uzyskać informacje o procesach, połączeniach sieciowych lub plikach, programy te pytają jądro systemu (kernbel) za pomocą wywołań systemowych (syscalls). Ponieważ rootkit działa wewnątrz jądra może przechwycić i modyfikować odpowiedzi. Dlatego też:
		...
		- jak naprawić: 
			...
			To jednak nie zawsze jest w stanie usunąć rootkita jądra, są sytuacje, w któych jedynym działającym rozwiązaniem jest przeinstalowanie całego systemu lub przywrócenie backupa. 
- `systemd`:
