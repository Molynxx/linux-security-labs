...
## What the FHS is
The file system on which Linux is based, is named FHS (File Hierarchy System). It is a standard which defines what each directory is used for. As a result independently pf the Linux distribution (Kali, Ubuntu, Debian, RHEL, Arch) we know that:
- logs are in `/var/log`, 
...
- `/lib` - libraries shared for programs form `/bin` and `/sbin`, 
- `/opt` - additional file system mount, 
...
- `/var/tmp` - temporary files (e.g. application's), which mus be saved between reboots, it is not clear upon reboot as `/tmp`,
- `/lib64` - in some distributions the `/lib64` exists for 64-bit libraries.

## Notice 
In modern distributions `/bin`, `/sbin`, `/lib` are often symbolic link to the proper directories in `/usr` (e.g. `/bin` -> `/usr/bin`).  