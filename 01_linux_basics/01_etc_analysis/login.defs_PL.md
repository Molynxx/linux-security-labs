...
- Polityka haseł:
	- `PASS_MAX_DAYS` - określa częstotliwość zmiany hasła, 
...
- Uprawnienia:
	- `UMASK` - domyślne uprawnienia dla nowo tworzonych plików (działa przez odejmowanie uprawnień, np. 077 daje 600). 
- Katalog domowy:
...
- Hashowanie haseł:
	...
	- `SHA_CRYPT_MAX_ROUNDS` - określa złożoność hashowania, niska wartość oznacza szybsze, ale także słabsze hashowanie, im wyższa wawrtość, tym bezpieczniej, lecz logowanie trwa dłużej.  
