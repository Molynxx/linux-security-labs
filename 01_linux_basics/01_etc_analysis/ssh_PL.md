...
Będąc przy temacie HostKey warto też wspomnieć o kluczach użytkownika. Klucze publiczne, które mogą logować się na konto użytkownika, przechowywane są w pliku systemu serwera:  
	`~/.ssh/authorized_keys` 
Jeśli atakujący uzyska dostęp do konta z uprawnieniami zapisu do tego pliku, może dodać własny klucz publiczny i uzyskać trwały dostęp do serwera bez znajomości hasła. Dlatego w analizie incydentów bezpieczeństwa jednym z pierwszych kroków jest sprawdzenie zawartości pliku `authorized_keys`. 
...
Jest to ważne, ponieważ jeśli zostanie popełniony błąd w match blocks, może niechcący otworzyć root na zewnątrz lub zablokować admina. Podczas analizy logów trzeba sprawdzić, który blok zadziałał dla danego połączenia. 
...
- UsePam yes, oznacza że SSH korzysta z mechanizmów PAM. SSH korzysta z PAM, dlatego ograniczenia logowania mogą wynikać z polityk PAM, a nie tylko z ustawień SSH.  
...
- zmienić port na mniej oczywisty, warto używać zakresu 49152 - 65535 żeby unikać kolizji i zmniejszyć ryzyko skanowania, 
...
- zmienić dozwoloną ilość prób logowania na 3, to dobry kompromis pomiędzy ochroną przed brute force a możliwości pomyłki przy wpisywaniu hasła przez użytkownika. 
...
- zmienić opcję PermitRootLogin na 'no', ponieważ gdy jest ustawiona grupa AllowUsers, której roota nie ma, jest ona niepotrzebna i nie ma wpływu na nic. Jednak nadal, gdyby ustawienia AllowUsers się w przyszłości zmieniły warto tą linią zabezpieczyć serwer dodatkowo, 
...
- log 3 - logowanie po 22 w nocy powinno budzić podejrzenia, zwłaszcza jeśli to nie są godziny pracy. Teoretycznie to mógł być admin pragnący przeprowadzić jakieś prace na serwerze poza godzinami pracy, to jeszcze nie jest powód do paniki, jednak przypadek wymaga dokładnego sprawdzenia.   
