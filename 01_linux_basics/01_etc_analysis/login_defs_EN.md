...
- `pam_unix.so` - determines the hash algorithm defined in the `ENCRYPT_METHOD` parameter and the complexity of hashing from the `SHA_CRYPT_MAX_ROUNDS` parameter, 
- `pam_faildelay.so` - reads the `FAIL_DELAY` parameter value, which determines the delay after a failed login attempt, 
...
- password policy:
	...
	- `PASS_WARN_AGE` - determines how many days before the password expires the system warns, thanks to this, users are informed about necessary password change.   
...
- users' accounts:
	...
	- `SYS_UID_MIN` / `SYS_UID_MAX` - analogously to the above, this is the UID range for system accounts, 
- `su` restriction:
	- `SU_WHEEL_ONLY` - set to 'yes' means that only the wheel group can use `su` , this is an essential layer of security against `su` attempts by non-privileged users. NOTE: in some distributions, `su` restriction to the wheel group is performed by the `pam_wheel.so` PAM module in the `/etc/pam.d/su` file, 
- privileges:
...
- change `UID_MIN` to the system range value so their account looks like a service and doesn't raise suspicion at first glance - the change applies to newly created accounts only,
- turn off the `SU_WHEEL_ONLY` and add their own account to the wheel group to be able to use `su`, 
- change `ENCRYPT_METHOD` to a weak hashing algorithm (e.g. DES) to easily crack passwords, 
- set `FAIL_DELAY=0` to speed up brute force attacks. 

## Security conclusions
Monitoring the essential parameters in the `/etc/login.defs` file is very important for unauthorized changes. For this purpose, the following commands are useful: 
- `grep -E "PASS_MAX_DAYS|PASS_MIN_DAYS|PASS_WARN_AGE" /etc/login.defs` - checking the password aging policy,
- `grep -E "UID_MIN|UID_MAX|SYS_UID_MIN|SYS_UID_MAX" /etc/login.defs` - checking UID ranges, 
