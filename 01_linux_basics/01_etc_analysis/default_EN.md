...
- threats:
	- added entry `init=/bin/bash` or `single` or `1` (single user mode) in `GRUB_CMDLINE_LINUX_DEFAULT`. These are parameters that starts the system in a single user mode, which also gives access to the root shell. In this mode the system start up without network services and minimal environment, but it gives an immediate root shell (without asking for the password). If an attacker has physical access this allow them to bypass the entire authentication mechanism using entry `single` or `1`. After restart the system start up straight away to the root's shell (without login and password). 
- detecting:
...
- detecting:
	- `/etc/default/cron` - the environment variable settings should be checked. (Correct environment variable settings is described in more detail in the file `05_pam_basics/security/pam_env_EN.md`). 
- repair:
