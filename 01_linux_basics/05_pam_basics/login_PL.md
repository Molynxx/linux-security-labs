...
Ta aplikacja odpowiedzialna jest za podstawowe, niskopoziomowe zabezpieczenie sytemu, które zabezpiecza lokalny dostęp do systemu i działa niezależnie od SSH i GUI. 
...
- moduły typu account - tym odpowiada za autoryzacje, więc znajdują się tutaj modułu sprawdzające czy użytkownik może się zalogować, 
...
- ważne jest sprawdzanie kto i kiedy zmienił plik nologin w katalogu `/etc/` za pomocą polecenia stat (`mtime`, `ctime`, `acces`).
