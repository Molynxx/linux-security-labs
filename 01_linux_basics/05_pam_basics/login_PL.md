...
Ta aplikacja odpowiedzialna jest za podstawowe, niskopoziomowe zabezpieczenie systemu, które zabezpiecza lokalny dostęp do systemu i działa niezależnie od SSH i GUI. Podczas gdy SSH umożliwia jedynie dostęp zdalny do systemu, login robi o wiele więcej:
- pyta o użytkownika, 
...
- flaga - tu rozszerzona flaga kontrolna, czyli zamiast required, requisite, sufficient, optional występuje:  
	...
	- `ignore=ignore`:  
		...
		to oznacza, ze wszystko jest w porządku, sesja może być kontynuowana,  
...
- Bardzo ważne jest sprawdzanie kto i kiedy zmienił plik nologin w katalogu `/etc/`` za pomocą polecenia stat (mtime, ctime, atime).
