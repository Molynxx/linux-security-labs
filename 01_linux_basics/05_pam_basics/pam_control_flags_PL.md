...
To są flagi pozwalają ujmować sytuacje warunkowe if/else. Struktura zapisu takiej flagi to:
...
`Default` zawsze obejmuje wszystkie nieprzewidziane wyniki.
...
- `pam_faillock.so` - powinien występować z flagą `required` by zapewnić ciągłość wykonania typu, bez względu na jego wynik oraz mieć ustawioną opcję authfail.
...
- zmienić kolejność modułów, by `pam_faillock.so` był jako pierwszy,
...
- w większości modułów nie ma potrzeby stosowania flag rozszerzonych, jedyna zasadna flaga rozszerzona jest dla modułu pam_unix.so.  
