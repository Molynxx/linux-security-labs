...
- greeter-show-manual-login - jeśli opcja nie jest ustawiona na false, umożliwia ręczne wpisanie nazwy użytkownika w tym root (jeśli PAM na to pozwala). 
...
- `pam_gnome_keyring.so` - (auth, session) 0 odblokowuje klucze GNOME po zalogowaniu, 
...
- polityka bezpieczeństwa  a konfiguracja pliku .conf:
	...
	- brak logowania jako gość - nie spełniony, ponieważ `allow-guest=true` oznacza, że opcja logowania jako gość jest włączona, 
	...
	- blokada po 5 nieudanych próbach logowania - nie spełniony - moduły w typie auth są ustawione prawidłowo, jednak z logów wynika, że prób na jednego użytkownika było 6. Przyczyna -> niewłaściwa konfiguracja pliku `/etc/pam.d/faillock.conf` (brak wpisu deny=5),
...
- polityka bezpieczeństwa:
	- zamienić autologin-user na pustą wartość, lub usunąć wpis autologin-user,
